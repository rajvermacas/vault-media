---
type: overview
status: active
updated: 2026-07-18
---

# Wiki index

This is the map of the maintained knowledge base. Read it before exploring
individual pages.

## Overview

- [[wiki/overview]] — Current shape, operating model, and open questions for this wiki.

## Concepts

- [[wiki/concepts/llm-wiki]] — The persistent-wiki pattern: sources are compiled into an evolving, interlinked synthesis.

## Sources

- [[wiki/sources/llm-wiki-idea]] — Seed design note describing the LLM Wiki pattern and its workflows.
- [[wiki/sources/reddit-city-as-character-series]] — Reddit-sourced series recommendations for settings that function as characters.

## Queries and analyses

- [[wiki/queries/city-as-character-series]] — Screened recommendations for the *Twin Peaks*/*Dark*/*Ozark*/*Banshee* atmosphere.
- [[wiki/queries/alexandra-daddario-movie-identification]] — Identification and viewing details for *The Layover*.

## Maintenance queue

- Decide which domains this vault will cover first.
- Run a lint pass after the first few ingests and refine [[AGENTS]] if needed.
